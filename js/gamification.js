...
class GamificationManager {
    ...
    getDefaultAchievements() {
        return [
            { id: 'first_habit', name: 'Langkah Pertama', description: 'Selesaikan habit pertama Anda', icon: '🎯', xpReward: 50, unlocked: false, category: 'habits' },
            { id: 'week_warrior', name: 'Pejuang 7 Hari', description: 'Selesaikan 7 habit secara konsisten', icon: '🏆', xpReward: 100, unlocked: false, category: 'habits' },
            { id: 'month_master', name: 'Master Bulanan', description: 'Selesaikan 30 habit dalam sebulan', icon: '👑', xpReward: 250, unlocked: false, category: 'habits' },
            { id: 'level_5', name: 'Pemula Berhasil', description: 'Capai level 5', icon: '📈', xpReward: 150, unlocked: false, category: 'level' },
            { id: 'level_15', name: 'Pejuang Sejati', description: 'Capai level 15', icon: '💪', xpReward: 300, unlocked: false, category: 'level' },
            { id: 'first_income', name: 'Pemasukan Pertama', description: 'Catat pemasukan pertama di finance tracker', icon: '💰', xpReward: 75, unlocked: false, category: 'finance' },
            { id: 'budget_master', name: 'Master Budget', description: 'Tetapkan budget untuk 5 kategori', icon: '📊', xpReward: 125, unlocked: false, category: 'finance' },
            { id: 'skill_learner', name: 'Pembelajar Aktif', description: 'Selesaikan skill pertama di learning path', icon: '📚', xpReward: 100, unlocked: false, category: 'learning' },
            { id: 'mood_tracker', name: 'Pencatat Suasana', description: 'Catat mood selama 7 hari berturut-turut', icon: '😊', xpReward: 80, unlocked: false, category: 'mental' },
            { id: 'clean_week', name: 'Minggu Bersih', description: '7 hari tanpa trigger negatif', icon: '🌟', xpReward: 200, unlocked: false, category: 'special' },
            { id: 'early_bird', name: 'Burung Pagi', description: 'Bangun sebelum jam 6 pagi selama 5 hari', icon: '🐦', xpReward: 120, unlocked: false, category: 'habits' },
            { id: 'financial_freedom', name: 'Menuju Freedom', description: 'Capai savings rate 30%', icon: '🏦', xpReward: 400, unlocked: false, category: 'finance' },
            { id: 'knowledge_seeker', name: 'Pencari Ilmu', description: 'Selesaikan 10 skill di learning path', icon: '🎓', xpReward: 350, unlocked: false, category: 'learning' },
            { id: 'mental_warrior', name: 'Prajurit Mental', description: 'Pertahankan mental health di atas 80% selama 14 hari', icon: '🧘', xpReward: 250, unlocked: false, category: 'mental' },
            { id: 'consistency_king', name: 'Raja Konsistensi', description: '100 hari streak habit', icon: '🔥', xpReward: 1000, unlocked: false, category: 'special' }
        ];
    }
    ...
    async addXP(amount, reason = '') {
        ...
        // This part is tricky as mainApp is not globally available anymore.
        // We will rely on re-rendering from the page-specific logic for now.
        // Example: The habit toggle in main.js calls loadDashboardData() after adding XP.
    }
    ...
    async unlockAchievement(achievementId) {
        ...
        this.storageManager.saveToLocalStorage('achievements', this.achievements);
    ...
    async checkHabitAchievements() {
        try {
            const logs = await this.storageManager.getAllFromDB('habitsDB', 'habitLogs');
            if (logs.length > 0) await this.unlockAchievement('first_habit');

            const completedThisWeek = this.getLogsInLastDays(logs, 7).filter(l => l.status === 'completed').length;
            if (completedThisWeek >= 7) await this.unlockAchievement('week_warrior');

            const completedThisMonth = this.getLogsInLastDays(logs, 30).filter(l => l.status === 'completed').length;
            if (completedThisMonth >= 30) await this.unlockAchievement('month_master');

            const earlyLogs = logs.filter(log => log.status === 'completed' && new Date(log.timestamp).getHours() < 6);
            if ([...new Set(earlyLogs.map(log => log.date))].length >= 5) await this.unlockAchievement('early_bird');
        } catch (error) {
    ...
    async checkFinanceAchievements() {
        try {
            const transactions = await this.storageManager.getAllFromDB('financeDB', 'transactions');
            if (transactions.some(t => t.type === 'income')) await this.unlockAchievement('first_income');

            const budgets = await this.storageManager.getAllFromDB('financeDB', 'budgets');
    ...
    async checkLearningAchievements() {
        try {
            const skills = await this.storageManager.getAllFromDB('learningDB', 'skills');
    ...
    async checkMentalAchievements() {
        try {
            const moodLogs = await this.storageManager.getAllFromDB('mentalDB', 'moodLogs');
            const last7Days = this.getLastNDates(7);
            if (last7Days.every(date => moodLogs.some(log => log.date === date))) {
                await this.unlockAchievement('mood_tracker');
            }

            const last14Days = this.getLastNDates(14);
            const highMoodDays = moodLogs.filter(log => last14Days.includes(log.date) && log.mood >= 4).length;
    ...
    async checkSpecialAchievements() {
        try {
            const reports = await this.storageManager.getAllFromDB('systemDB', 'pornReports');
            const last7Days = this.getLastNDates(7);
            const hasReportsInLast7Days = reports.some(r => last7Days.includes(new Date(r.timestamp).toISOString().split('T')[0]));
            if (!hasReportsInLast7Days && reports.length > 0) await this.unlockAchievement('clean_week');

            const streaks = await this.storageManager.getAllFromDB('habitsDB', 'streaks');
            const maxStreak = streaks.reduce((max, s) => Math.max(max, s.bestStreak || 0), 0);
    ...
    getLogsInLastDays(logs, days) {
        ...
        return logs.filter(log => new Date(log.date || log.timestamp) >= cutoffDate);
    }
    ...
    getTotalAchievementXP() {
        return this.achievements.filter(a => a.unlocked).reduce((total, a) => total + a.xpReward, 0);
    }
}
